...
- **Memory Engine 2.0**：在兼容原角色摘要的基础上，结构化保存事件、承诺、秘密、信念、计划、关系和未决事项，并按角色知情范围与当前话题检索。
...
V7.0 将原动态摘要升级为 Memory Engine 2.0，同时保留按角色组织的旧摘要文件和摘要管理界面。主要目标是：
...
- 通过 lazy adapter 兼容旧摘要，不强制批量重写用户数据。
- 摘要管理页同时展示 Memory Engine 2.0 覆盖情况和旧摘要兼容记录；按人物姓名或 ID 可精确定位本人目录，并汇总其他人物目录中的直接对话和相关提及。
...
├─ resources/app/out/main/memory-system/             # Memory Engine 2.0
...
- 当前重点：Memory Engine 2.0 的知识边界、长期记忆检索、失败恢复，以及模块化动作／会话运行时
