...
V7.8 继续使用 Memory Engine 2.4。旧摘要导入、跨周目摘要提示和旧格式运行路径已退休；人物目录是唯一摘要正文来源。主要规则是：
...
清单会覆盖全部 `test-*.js`：直接发布组、动作聚合组和 follow-up 聚合组均必须登记。V7.8 发布门禁为 38 组、覆盖 71 个测试文件，并维护 36 条动作/非动作/摘要金标样例，作为 Provider 切换或模型升级时的轻量人工语义复核基线。
...
- 当前重点：V7.8 / Pre-V8 主进程模块化边界、行为等价回归与历史 Provider 可替换接口
