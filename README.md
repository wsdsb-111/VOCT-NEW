...
- **Memory Engine 2.1**：以 `角色ID_姓名/与对方的对话.json` 人物目录为唯一可见记忆层，结构化索引仅在内部维护知识边界；每个回应角色只从自己的目录检索直接参与者和任意数量被提及人物的相关摘要。
...
V7.1 将人物摘要目录正式纳入 Memory Engine 2.1，不再让结构化记忆树与旧摘要界面并行显示。主要规则是：
...
- 结合重要度、实体、话题、关系、时间和可信度进行本地检索；
- 控制提示词长度，减少不必要的 token 消耗；
- A 与 B 对话时，A、B 分别从自己的摘要目录读取双方历史；任何回应角色都不会借用另一角色的私人目录；
- 对话提到 C/E 等第三者时，每个当前回应角色分别在自己的目录中检索相关摘要，参与者与提及人物没有固定数量上限，但仍受动态 token budget 约束；
...
├─ resources/app/out/main/memory-system/             # Memory Engine 2.1
...
- 当前重点：Memory Engine 2.1 的人物目录召回、多人视角隔离、最终摘要事务恢复，以及模块化动作／会话运行时
...
- 人物摘要目录是 Memory Engine 2.1 的可见长期记忆数据；清理或迁移前请先备份 `%APPDATA%/VOTC/votc_data/conversation_summaries`。
