...
V7.8.3 使用 Memory Engine 2.5。旧摘要导入、跨周目摘要提示和旧格式运行路径已退休；人物目录仍是唯一摘要正文来源，2.3/2.4 视角摘要继续兼容读取。主要规则是：
...
清单会覆盖全部 `test-*.js`：直接发布组、动作聚合组和 follow-up 聚合组均必须登记。V7.8.3 发布门禁为 46 个直接回归组，清单覆盖 79 个测试文件，并维护 36 条动作/非动作/摘要金标样例，作为 Provider 切换或模型升级时的轻量人工语义复核基线。
...
- 当前重点：V7.8.3 / Memory Engine 2.5 Turn Recall、动作精度与全在场关系权威层
