...
- **Memory Engine 2.3**：以 `角色ID_姓名/与对方的对话.json` 人物目录为唯一可见记忆层；终局按 `owner × counterpart` 的知情与主题边界生成配对视角摘要，召回在同一场对话内冻结直接关系与场外人物快照。
- **候场与在场窗口**：多人会话可在首句前设置候场，开始后用“请入内/请离场”调整实际参与者；每名角色只回应、获知并保存自己实际在场区间的内容。
...
V7.7.2 使用 Memory Engine 2.3。旧摘要导入、跨周目摘要提示和旧格式运行路径已退休；人物目录是唯一摘要正文来源。主要规则是：

- 摘要 Provider 失败时不推进 rolling checkpoint，并用 recovery snapshot 保留最终失败的原始会话；
- 结构化区分事实、信念、计划、传闻、秘密、承诺和关系变化；
- 每个 `owner → counterpart` 文件只接收 owner 已知且与 counterpart 主题相关的记忆；只与第三人有关的事件不会复制到其他配对文件；
...
- 开场仍默认所有已选 NPC 在场；首句前可将任一非最后在场 NPC 设为候场。候场者不进入回应队列、不读取本轮记忆，也不会因从未入场而获得终局摘要；
- “请入内”从入内系统消息开始打开人物窗口且当轮不插队补话；“请离场”以离场系统消息为窗口右边界并立即生成离场前摘要，离场标记及其后的正文不会进入该人物视角；
...
- 直接关系、场外人物和稳定记忆只在实际命中候选时占用预算；当前话题最多补入 1 条后缀记忆，不改变已冻结前缀；
- 人物提及统一使用同一匹配器：唯一称号可以指代人物、同名或同称号歧义失败关闭并记录诊断、中文长名优先，“那个人”绑定本场最近的明确第三人称对象；信件也走同一 Engine 2.3 路由；
...
- 摘要管理页只展示一套“人物目录 → 与某角色的对话 → 可编辑摘要”树；按姓名或 ID 搜索时，本人目录排在首位，并继续列出其他目录中的直接对话与相关提及；
...
├─ resources/app/out/main/memory-system/             # Memory Engine 2.3
...
清单会覆盖全部 `test-*.js`：直接发布组、动作聚合组和 follow-up 聚合组均必须登记。V7.7.2 发布门禁为 35 组，并维护 36 条动作/非动作/摘要金标样例，作为 Provider 切换或模型升级时的轻量人工语义复核基线。
...
- 当前重点：V7.7.2 候场、入内、主动离场与人物实际在场窗口
...
- 人物摘要目录是 Memory Engine 2.3 的可见长期记忆数据；清理或迁移前请先备份 `%APPDATA%/VOTC/votc_data/conversation_summaries`。
