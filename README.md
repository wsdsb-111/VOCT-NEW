...
- **Memory Engine 2.4**：以 `角色ID_姓名/与对方的对话.json` 人物目录为唯一可见记忆层；终局按 `owner × counterpart` 的知情与主题边界生成配对视角摘要，人物目录按角色缓存并在写入后失效，模型引用的来源消息 ID 必须通过真实历史校验。
...
V7.7.3 使用 Memory Engine 2.4。旧摘要导入、跨周目摘要提示和旧格式运行路径已退休；人物目录是唯一摘要正文来源。主要规则是：
...
- 人物提及统一使用同一匹配器：唯一称号可以指代人物、同名或同称号歧义失败关闭并记录诊断、中文长名优先，“那个人”绑定本场最近的明确第三人称对象；信件也走同一 Engine 2.4 路由；
...
├─ resources/app/out/main/memory-system/             # Memory Engine 2.4
...
清单会覆盖全部 `test-*.js`：直接发布组、动作聚合组和 follow-up 聚合组均必须登记。V7.7.3 发布门禁为 36 组，并维护 36 条动作/非动作/摘要金标样例，作为 Provider 切换或模型升级时的轻量人工语义复核基线。
...
- 当前重点：V7.7.3 / Memory Engine 2.4 摘要读取缓存、来源 messageId 可信校验与统一管理入口
...
- 人物摘要目录是 Memory Engine 2.4 的可见长期记忆数据；清理或迁移前请先备份 `%APPDATA%/VOTC/votc_data/conversation_summaries`。
