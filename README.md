...
V7.7.1 使用 Memory Engine 2.3。旧摘要导入、跨周目摘要提示和旧格式运行路径已退休；人物目录是唯一摘要正文来源。主要规则是：
...
清单会覆盖全部 `test-*.js`：直接发布组、动作聚合组和 follow-up 聚合组均必须登记。V7.7.1 发布门禁为 34 组，并维护 36 条动作/非动作/摘要金标样例，作为 Provider 切换或模型升级时的轻量人工语义复核基线。
...
- 当前重点：V7.7.1 Memory Engine 2.3 视角摘要、会话冻结召回和终局可靠性
