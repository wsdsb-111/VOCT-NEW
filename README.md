...
- **Memory Engine 2.2**：以 `角色ID_姓名/与对方的对话.json` 人物目录为唯一可见记忆层；每个 NPC 先精确读取自己目录中的直接关系文件，再从自己的目录检索任意数量被提及场外人物的相关摘要。
...
- 同一场对话内的长期稳定记忆使用与问题和在场名单无关的固定排序，保持每轮提示词前缀一致；对话结束后的新摘要在下一场会话重新读取；
- A 与 B 对话时，NPC 默认读取自己目录中与对方最近 2 条及当前话题最相关 1 条摘要；玩家不执行记忆提示词注入；
- 三人以上对话先为每个其他在场人物保证 1 条直接关系记忆，再补充全局最相关 2 条，避免某一人物垄断预算；
...
- NPC 死亡后立即删除该 NPC 自己拥有的全部摘要目录，其他人物目录中关于死者的摘要继续保留；终局恢复不会重新创建死者目录；
