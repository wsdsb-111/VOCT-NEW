...
V7.2 将 Memory Engine 升级到 2.2，在 V7.1 的统一人物目录基础上加入精确关系路由、全发言者提及识别和串行终局提交。主要规则是：
...
- 玩家或任一 NPC 提到 C/E 等场外人物后，所有后续 NPC 分别从自己的目录读取该人物最近 1 条、预算允许时再补充相关 1 条；
...
- 会话结束改由串行终局协调器提交；立即开始第二场对话不会复用上一场状态，退出应用前会等待已进入队列的摘要任务完成；
...
- 当前重点：Memory Engine 2.2 的精确人物目录路由、全发言者场外人物召回、连续会话终局可靠性及动作隔离
