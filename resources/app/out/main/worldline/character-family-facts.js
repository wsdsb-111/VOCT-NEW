...
function formatStructuredCharacter(character = {}, currentGameDate = null, characters = null) {
  const id = String(character.id ?? "");
  const name = characterName(character, id);
  const sex = resolveCharacterSex({ snapshot: character });
  const age = resolveCharacterAge(character, currentGameDate);
  const death = formatDeathFact(character, { currentGameDate, characters });
...
function buildFamilyFactBlock(character, gameData) {
  ...
  for (const edge of relations) {
    ...
    const death = formatDeathFact(relative, { currentGameDate: gameData.date, characters: graph.nodes });
    const age = resolveCharacterAge(relative, gameData.date);
    const details = [];
