...
Keep action detection deterministic before any model request. Explicitly distinguish completed narration from questions, commands, plans, hypothetical statements, failed attempts, memories, and discussion. When changing action candidates, preserve the chain: gate → semantic shortlist → registered action check → structured model response → local validation.
