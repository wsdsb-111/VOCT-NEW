...
class MemoryExtractor {
  buildPrompt({ messages, participants, participantPresence = [], date, totalDays, rollingSummary = "", finalInstructions = "" } = {}) {
    ...
    return [
      {
        role: "system",
        content: `VOTC Memory Engine ${MEMORY_ENGINE_VERSION} structured extraction. Return one JSON object only; never put analysis or reasoning inside the JSON. Do not decide who knows a memory; knowledge is computed locally. Distinguish facts, beliefs, plans and rumors. Split memories when different people are the subject. participants are the people who acted in or directly took part in that event; subjects are only the people directly described by that memory. Do not copy every scene participant into subjects. Respect every supplied half-open presence window: a character can participate in or hear message M only when at least one of that character's windows satisfies joinedAt <= M.id < leftBefore. Multiple windows for one character mean temporary absence; do not give that character any summary or memory content sourced from the gaps between those windows. A presence_temporary_return message may establish only the stated fact that the character was unconscious, asleep or away and has now returned; it does not reveal anything that happened during the gap. Never include a waiting character before joinedAt or a permanently departed character at/after leftBefore. Always provide the exact supporting messageIds.

Put the complete detailed narrative in summarySegments. There is no fixed character or word count; use the configured output-token budget according to the amount of source detail, without padding or arbitrary compression. Be as concise as possible without losing substantive content, character attribution, causal links or key details; remove repetition and low-information wording instead of removing facts. Use dense chronological segments and start a new segment when the present-character set, scene/topic, or knowledge audience materially changes. Every summarySegment must stay inside one participant-presence window: never combine supporting messageIds across join, leave, temporary-leave, or return boundaries, even when the same participants are present again later. Split private thoughts, silent intentions, self-talk and unobserved actions from shared events. If a character is asleep, unconscious, absent or has left, do not include that character among the participants or speakers of content they could not perceive; mark the segment private or known_group as appropriate so local knowledge projection cannot expose it. Each segment must preserve the scene and causal sequence; attribute every action, statement, belief and emotion to the correct named character; and retain exact decisions, promises, conditions, refusals, requests, secrets, plans, conflicts, relationship changes, emotional and tone shifts, unresolved matters and outcomes; and exact numbers, dates, locations, titles, objects and quoted terms. Never replace concrete details with generic phrases such as 'they discussed the matter'. Every segment must list all and only its exact supporting messageIds. Do not repeat the same narrative in a separate sessionSummary; the application joins summarySegments locally so output budget is spent on detail instead of duplicate prose.
