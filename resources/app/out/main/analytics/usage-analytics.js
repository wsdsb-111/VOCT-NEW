  ...
  class UsageAnalytics {
    ...
    record(metadata, usage) {
      ...
      const entry = {
        ...
        memoryMentionedSnapshotTokens: Number(metadata?.memoryMentionedSnapshotTokens ?? metadata?.mentionedSnapshotTokens) || 0,
        mentionedSnapshotTokens: Number(metadata?.mentionedSnapshotTokens) || 0,
        memoryTurnRecallTokens: Number(metadata?.memoryTurnRecallTokens) || 0,
        turnRecallTokens: Number(metadata?.turnRecallTokens ?? metadata?.memoryTurnRecallTokens) || 0,
        thirdPartyEvidenceTokens: Number(metadata?.thirdPartyEvidenceTokens) || 0,
        totalMemoryTokens: Number(metadata?.totalMemoryTokens) || 0,
        worldStableTokens: Number(metadata?.worldStableTokens) || 0,
        worldTurnRecallTokens: Number(metadata?.worldTurnRecallTokens) || 0,
        worldlineTokens: Number(metadata?.worldlineTokens) || 0,
        ...
        worldQueryIntent: metadata?.queryIntent || null,
        whereaboutsBlockedCount: Number(metadata?.whereaboutsBlockedCount) || 0,
        worldFactsRendered: Number(metadata?.worldFactsRendered) || 0,
        coveragePhase: metadata?.coveragePhase || null,
        coverageIntent: metadata?.coverageIntent || null,
        coverageHit: metadata?.coverageHit === undefined ? null : metadata.coverageHit === true,
        coverageMissingFields: Number(metadata?.coverageMissingFields) || 0,
        coverageRequestedFields: Array.isArray(metadata?.coverageRequestedFields) ? metadata.coverageRequestedFields.map(String).slice(0, 8) : [],
        coverageLaneCoverage: metadata?.coverageLaneCoverage && typeof metadata.coverageLaneCoverage === "object" ? Object.fromEntries(Object.entries(metadata.coverageLaneCoverage).slice(0, 12).map(([lane, value]) => [lane, {
          candidateCount: Number(value?.candidateCount) || 0, selectedCount: Number(value?.selectedCount) || 0, complete: value?.complete === true
        }])) : null,
        baselineFactCount: Number(metadata?.baselineFactCount) || 0,
        baselineTokens: Number(metadata?.baselineTokens) || 0,
        baselineLanes: metadata?.baselineLanes || null,
        baselineLaneCoverage: metadata?.baselineLaneCoverage && typeof metadata.baselineLaneCoverage === "object" ? Object.fromEntries(Object.entries(metadata.baselineLaneCoverage).slice(0, 12).map(([lane, value]) => [lane, {
          candidateCount: Number(value?.candidateCount) || 0, selectedCount: Number(value?.selectedCount) || 0, complete: value?.complete === true
        }])) : null,
        baselineTruncated: metadata?.baselineTruncated === true,
        patchCacheHit: metadata?.patchCacheHit === true,
        patchFactCount: Number(metadata?.patchFactCount) || 0,
        patchCandidateCount: Number(metadata?.patchCandidateCount) || 0,
        patchSelectedCount: Number(metadata?.patchSelectedCount) || 0,
        patchTruncated: metadata?.patchTruncated === true,
        patchTokens: Number(metadata?.patchTokens) || 0,
        patchFilteredCount: Number(metadata?.patchFilteredCount) || 0,
        ...
        temporalDiagnostics: metadata?.temporalDiagnostics && typeof metadata.temporalDiagnostics === "object" ? {
          ...
          axis: String(metadata.temporalDiagnostics.axis || "none"),
          expression: typeof metadata.temporalDiagnostics.expression === "string" ? metadata.temporalDiagnostics.expression.slice(0, 80) : null,
          ownerId: Number.isSafeInteger(Number(metadata.temporalDiagnostics.ownerId)) ? Number(metadata.temporalDiagnostics.ownerId) : null,
          querySpeakerId: metadata.temporalDiagnostics.querySpeakerId != null && Number.isSafeInteger(Number(metadata.temporalDiagnostics.querySpeakerId)) ? Number(metadata.temporalDiagnostics.querySpeakerId) : null,
          directCounterpartIds: Array.isArray(metadata.temporalDiagnostics.directCounterpartIds) ? metadata.temporalDiagnostics.directCounterpartIds.map(Number).filter(Number.isSafeInteger).slice(0, 8) : [],
          currentGameDate: metadata.temporalDiagnostics.currentGameDate || null,
          targetGameYear: Number.isInteger(metadata.temporalDiagnostics.targetGameYear) ? metadata.temporalDiagnostics.targetGameYear : null,
          folderMemoryCount: Number(metadata.temporalDiagnostics.folderMemoryCount) || 0,
          campaignAcceptedCount: Number(metadata.temporalDiagnostics.campaignAcceptedCount) || 0,
          campaignRejectedCount: Number(metadata.temporalDiagnostics.campaignRejectedCount) || 0,
          legacyCampaignRejectedCount: Number(metadata.temporalDiagnostics.legacyCampaignRejectedCount) || 0,
          conversationTimeCandidates: Number(metadata.temporalDiagnostics.conversationTimeCandidates) || 0,
          eventTimeCandidates: Number(metadata.temporalDiagnostics.eventTimeCandidates) || 0,
          conversationHits: Number(metadata.temporalDiagnostics.conversationHits) || 0,
          eventHits: Number(metadata.temporalDiagnostics.eventHits) || 0,
          seenSuppressedCount: Number(metadata.temporalDiagnostics.seenSuppressedCount) || 0,
          budgetOmittedCount: Number(metadata.temporalDiagnostics.budgetOmittedCount) || 0,
          selectedCount: Number(metadata.temporalDiagnostics.selectedCount) || 0,
          selectedSummaryIds: Array.isArray(metadata.temporalDiagnostics.selectedSummaryIds) ? metadata.temporalDiagnostics.selectedSummaryIds.map(String).filter(Boolean).slice(0, 3) : []
        } : null,
