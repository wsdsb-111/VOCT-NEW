...
function detect(text, { candidateOnly = false } = {}, { registry } = {}) {
    ...
    const rules = [
      ...
      { reason: "sexual_intercourse_completed", pattern: /(?:(?:已经|已)(?:同房|圆房|行房|完成房事)|完成(?:了)?(?:交合|行房|房事|同房|圆房|性事)|发生(?:了)?(?:性关系|肉体关系)|一番云雨之后|云雨(?:已)?(?:毕|罢)|行过房事|共度(?:了)?春宵之后|鱼水之欢已毕|had (?:sexual )?intercourse|had sex|made love|consummated)/i }
    ];
    const completedSexualAction = /(?:(?:已经|已)(?:同房|圆房|行房|完成房事)|完成(?:了)?(?:交合|行房|房事|同房|圆房|性事)|发生(?:了)?(?:性关系|肉体关系)|一番云雨之后|云雨(?:已)?(?:毕|罢)|行过房事|共度(?:了)?春宵之后|鱼水之欢已毕|had (?:sexual )?intercourse|had sex|made love|consummated)/i;
    const detected = [];
