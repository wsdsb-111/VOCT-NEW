...
| 版本 | 重点 | 详细记录 |
| --- | --- | --- |
| v7.10.0-RC6 Candidate | Letter 新增 Authoritative Delivery Base：Payload/Tracker 漂移超过一天时强制对账并以 Tracker 重算投递日；正式 Outbound 固定为 `votc_run_file`。A3 升级为显式 `root` scope 的 Official-Parity Artifact，增加 PRE/POST/Scope Marker 与四类失败；Inbound 增加逐次安全诊断、`PAYLOAD_INCOMPLETE_TIMEOUT` 和同 trigger 手动重读。真实 A3/B/C/D、delay=0/3、stale payload 与 Popup Gate 通过前不标 Stable | [实施报告](docs/v7.10-official-action-letter-recovery-implementation-report.md) |
