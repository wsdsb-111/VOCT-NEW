...
(async () => {
  ...
  try {
    ...
    assert.equal(resolveHistoricalKnowledge({ projection: projection1145, responderId: "3", subjectId: "1", field: "LOCATION" }).reason, "HISTORICAL_COURT");
    ...
    const remembered = retrieveHistorical({ store, scope, queryPlan: { ...plan, time: { mode: "AS_OF", from: "1145.1.1", to: null }, entities: plan.entities }, query: "岳飞在哪里", currentDate: "1150.1.1", responderId: "6", memoryFacts: [{ ownerId: "6", knownBy: ["6"], entityId: "1", field: "LOCATION", structured: true, sourceTier: "PERSONAL_MEMORY", asOf: "1144.1.1" }] });
    assert(remembered.selected.some(item => item.field === "LOCATION" && item.knowledgeReason === "HISTORICAL_PERSONAL_MEMORY"));
    const observed = retrieveHistorical({ store, scope, queryPlan: { ...plan, time: { mode: "AS_OF", from: "1145.1.1", to: null }, entities: plan.entities }, query: "岳飞在哪里", currentDate: "1150.1.1", responderId: "6", directObservationFacts: [{ factId: "obs-1", entityId: "1", field: "LOCATION", knowledgeLevel: "DIRECT_OBSERVATION", directObserverIds: ["6"], asOf: "1144.1.1" }] });
