...
(async () => {
  ...
  try {
    ...
    const remembered = retrieveHistorical({ store, scope, queryPlan: { ...plan, time: { mode: "AS_OF", from: "1145.1.1", to: null }, entities: plan.entities }, query: "岳飞在哪里", currentDate: "1150.1.1", responderId: "6", memoryFacts: [{ ownerId: "6", knownBy: ["6"], participantIds: ["1"], asOf: "1144.1.1" }] });
    assert(remembered.selected.some(item => item.field === "LOCATION" && item.knowledgeReason === "HISTORICAL_PERSONAL_MEMORY"));
    const ambiguous = retrieveHistorical({ store, scope, queryPlan: { ...plan, entities: { ...plan.entities, characters: ["1", "2"] } }, query: "这个人在哪里", currentDate: "1150.1.1", responderId: "4" });
