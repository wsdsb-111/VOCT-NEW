...
const snapshot = {
  characters: {
    "100": { id: "100", firstName: "yuefei_name", alive: true, location: "8841" },
...
function findLocalizedKeys(type, value) {
  const values = {
    "character:岳飞": [{ rawKey: "yuefei_name" }],
...
assert.equal(chineseCharacter.characters[0].displayName, "岳飞", "localized character display text must remain separate from raw identity");
...
assert.ok(analysisTextMatches(chineseCharacter, "岳飞已抵达前线"), "Supplemental matching must reuse the analyzed entity terms");
