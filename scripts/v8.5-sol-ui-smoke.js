  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      window.worldlineAPI = {
        ...
        getPromptDiagnostics: async payload => ({ promptDiagnostics: { available: true, query: payload.query, queryAnalysis: { identityResolution: { status: "AMBIGUOUS", reason: "MULTIPLE_CANDIDATES", candidates: [{ runtimeId: "96895", definitionId: "tangyin_yue_014", rawName: "Fei_name11", score: 0.77, evidence: [{code:"NAME_EXACT"}], conflicts: [] }, { runtimeId: "96896", definitionId: "nansong_yue_085", rawName: "飞", score: 0.77, evidence: [{code:"AGE_MATCH_STRONG"}], conflicts: [] }] }, historicalCoverage: [{status:"AMBIGUOUS", reason:"MULTIPLE_CANDIDATES"}] }, historicalIndex: {status:"READY",sourceComplete:true}, gameTruth: {}, tokenBreakdown: [],
    ...
    await page.screenshot({ path: path.join(__dirname, "../output/playwright/v8.5.1-diagnostics.png"), fullPage: true });
