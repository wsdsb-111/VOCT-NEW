...
try {
  ...
  const promptBuild = PromptBuilder.buildMessagesWithTokenCount(
    [{ role: "assistant", content: "此前对话" }, { role: "user", content: "乙在哪里" }],
    promptCharacter,
    promptGameData,
    "",
    { engineVersion: "2.5", activeParticipantIds: [1], stableText: "绝密计划正文", turnRecallText: "本轮 Memory Turn Recall", worldStableText: owner.worldStableText, worldTurnRecallText: owner.worldTurnRecallText, historicalReferenceInfo: owner.historicalReferenceInfo }
  );
  const providerPrompt = promptBuild.messages.map((message) => message.content).join("\n");
  const promptBlockIds = promptBuild.blocks.map((entry) => entry.block.id);
  assert(promptBlockIds.indexOf("memory-stable") < promptBlockIds.findIndex((id) => id === "history"), "stable Memory must remain before prior conversation history");
  assert(promptBlockIds.findIndex((id) => id === "history") < promptBlockIds.indexOf("history-current-user"), "prior conversation history must remain before the current user");
  assert(promptBlockIds.indexOf("history-current-user") < promptBlockIds.indexOf("memory-turn-recall"), "Memory Turn Recall must follow the current user");
  assert(promptBlockIds.indexOf("memory-turn-recall") < promptBlockIds.indexOf("worldline-turn-recall"), "Worldline Turn Recall must follow Memory Turn Recall");
  assert(promptBlockIds.indexOf("history-current-user") < promptBlockIds.indexOf("worldline-turn-recall"), "Worldline Turn Recall must follow the current user");
  assert.equal(promptBuild.blocks.filter((entry) => entry.block.id === "worldline-turn-recall").length, 1, "the actual PromptBuilder emits exactly one Worldline Turn Recall block");
  ...
  const uninformedPromptBuild = PromptBuilder.buildMessagesWithTokenCount(
    ...
    promptGameData,
    "",
    { engineVersion: "2.5", activeParticipantIds: [4], worldStableText: uninformedPeer.worldStableText, worldTurnRecallText: uninformedPeer.worldTurnRecallText, historicalReferenceInfo: uninformedPeer.historicalReferenceInfo }
  );
