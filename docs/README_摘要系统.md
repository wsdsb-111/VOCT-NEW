# 对话摘要系统：Memory Engine 2.5（V7.8.3）

V7.8.3 以人物摘要文件夹作为唯一可见、可搜索、可编辑的长期记忆层。Memory Engine 2.5 保留 2.4 的结构化事件、`owner × counterpart` 知情/主题/在场窗口投影及目录缓存，并新增整场冻结的 Session Topic Anchor 与按轮动态 Turn Recall。旧 2.3/2.4 摘要继续兼容读取，不要求迁移。

## Memory Engine 2.5 动态召回
...
V7.8.3 状态面板标记 Memory Engine 2.5；人物目录、缓存、来源消息校验、多段在场窗口和最终摘要 Token 设置继续沿用 2.4 合同。摘要管理操作仍统一为同一标题栏中的“刷新 / 打开摘要文件夹 / 清除全部摘要”。
