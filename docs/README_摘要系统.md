...
- 兼容旧操作：创建会话时，CK3 已选择的 NPC 默认全部在场。首句发送前可用“设为候场”调整开场名单。
- 候场人物不进入回应队列、不读取当前对话记忆，也不写入 `knownBy`；若整场从未入内，则不属于本场摘要参与者。
- “请入内”插入 `【人物入内】` 系统消息，并从该消息 ID 打开人物窗口；入内当轮不热插队回复，从下一轮正常参与。
- “请离场”插入 `【人物离场】` 系统消息，以该消息 ID 关闭人物窗口并立即发起离场前摘要。窗口使用 `[joinedAtMessageId, leftAtMessageId)`，所以离场标记本身及后续对话都不会进入离场人物的摘要。
...
V7.7.2 状态面板展示 Memory Engine 2.3 配对视角摘要、稳定前缀、直接关系、多人、场外人物会话快照、人物在场窗口和 Token 边界。聊天窗口直接提供“设为候场/请入内/请离场”。普通 DeepSeek 对话及结构化终局摘要启用思考模式并使用 4096 Token；动作判定保持非思考。聊天统计额外记录 history 前聚合指纹；终局结构化正文无效时自动重试，不把思考残片当成摘要提交。
