...
V7.7.1 以人物摘要文件夹作为唯一可见、可搜索、可编辑的长期记忆层。Memory Engine 2.3 先在内部生成一份结构化事件，再按每名目录所有者的知情边界投影视角摘要，避免把同场其他角色的秘密复制到不知情者目录。旧摘要导入、跨周目导入通知和旧格式运行路径均已退休。
...
V7.7.1 状态面板展示 Memory Engine 2.3 视角摘要、稳定前缀、直接关系、多人、场外人物会话快照和 Token 边界。普通 DeepSeek 对话及结构化终局摘要启用思考模式并使用 4096 Token；动作判定保持非思考。
