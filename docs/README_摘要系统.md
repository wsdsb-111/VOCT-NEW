# 对话摘要系统：Memory Engine 2.3（V7.7.1）

V7.7.1 以人物摘要文件夹作为唯一可见、可搜索、可编辑的长期记忆层。Memory Engine 2.3 先在内部生成一份结构化事件，再按 `owner × counterpart` 同时应用知情边界和主题边界：`A/与B的对话.json` 不会混入只与 C 有关的事件。旧摘要导入、跨周目导入通知和旧格式运行路径均已退休。
...
信件与对话使用同一个 Engine 2.3 路由。收信人从自己目录读取与发信人的直接摘要，信件正文提到场外人物时，也从收信人自己的目录检索相关摘要。
...
V7.7.1 状态面板展示 Memory Engine 2.3 配对视角摘要、稳定前缀、直接关系、多人、场外人物会话快照和 Token 边界。普通 DeepSeek 对话及结构化终局摘要启用思考模式并使用 4096 Token；动作判定保持非思考。聊天统计额外记录 history 前聚合指纹；终局结构化正文无效时自动重试，不把思考残片当成摘要提交。
