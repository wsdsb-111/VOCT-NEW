# 对话摘要系统：Memory Engine 2.4（V7.8）

V7.8 以人物摘要文件夹作为唯一可见、可搜索、可编辑的长期记忆层。Memory Engine 2.4 先在内部生成一份结构化事件，再按 `owner × counterpart` 同时应用知情边界、主题边界和人物实际在场窗口：`A/与B的对话.json` 不会混入只与 C 有关或 A/B 未共同在场时发生的事件。2.4 的人物目录缓存可显示命中、未命中、失效和条目数；摘要页“刷新”会先清空缓存再重新扫描。v7.8 只迁移主进程模块边界，不改变摘要格式和召回逻辑；旧 2.3 摘要继续兼容读取，不要求迁移。
...
V7.8 状态面板展示 Memory Engine 2.4 配对视角摘要、稳定前缀、人物目录缓存、来源消息校验、场外人物会话快照、人物在场窗口和 Token 边界。摘要管理操作统一为同一标题栏中的“刷新 / 打开摘要文件夹 / 清除全部摘要”；“刷新”会同时令 Memory Engine 的人物目录缓存失效。普通 DeepSeek 对话启用思考模式并使用 4096 Token；结构化 `final_summary`、失败重试与 `memory_recovery` 均关闭思考，最终摘要的输出上限可在摘要管理页设置为 256–16384 Token（默认 4096）。
