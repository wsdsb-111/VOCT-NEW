# 对话摘要系统：Memory Engine 2.5（V7.9）

V7.9 继续以人物摘要文件夹作为唯一可见、可搜索、可编辑的长期记忆层，本次不改 Memory Engine 2.5。它保留 2.4 的结构化事件、`owner × counterpart` 知情/主题/在场窗口投影及目录缓存，以及整场冻结的 Session Topic Anchor 与按轮动态 Turn Recall。旧 2.3/2.4 摘要继续兼容读取，不要求迁移。
...
V7.9 状态面板继续标记 Memory Engine 2.5；人物目录、缓存、来源消息校验、多段在场窗口和最终摘要 Token 设置保持原合同。摘要管理操作仍统一为同一标题栏中的“刷新 / 打开摘要文件夹 / 清除全部摘要”。
