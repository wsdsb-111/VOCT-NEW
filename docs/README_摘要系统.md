# 对话摘要系统：Memory Engine 2.3（V7.7.2）

V7.7.2 以人物摘要文件夹作为唯一可见、可搜索、可编辑的长期记忆层。Memory Engine 2.3 先在内部生成一份结构化事件，再按 `owner × counterpart` 同时应用知情边界、主题边界和人物实际在场窗口：`A/与B的对话.json` 不会混入只与 C 有关或 A/B 未共同在场时发生的事件。旧摘要导入、跨周目导入通知和旧格式运行路径均已退休。
...
V7.7.2 状态面板展示 Memory Engine 2.3 配对视角摘要、稳定前缀、直接关系、多人、场外人物会话快照、人物在场窗口和 Token 边界。聊天窗口直接提供“设为候场/请入内/请离场”。普通 DeepSeek 对话启用思考模式并使用 4096 Token；结构化 `final_summary`、失败重试与 `memory_recovery` 全部使用 4096 Token 非思考输出，避免隐藏推理耗尽正文。

终局 Provider 将完整叙事写为带原始 `messageIds` 的 `summarySegments`，应用本地拼成最终摘要，并按双方共同知情、共同在场的窗口投影到 `owner → counterpart` 文件。这样详细正文不再只留在内部 episode，也不会被长期记忆索引压缩成一两句话。摘要与单条长期记忆均不设置固定字数或段落数，模型根据原始信息量使用 4096 Token Provider 输出预算；缺少 `summarySegments`、正文为空或任一分段没有有效来源消息时，即使 Provider 返回 `finish_reason=stop` 也拒绝提交，并在第二次请求中明确要求修复结构和细节。摘要必须逐人归属言行、观点与情绪，保留时间地点、数字物件、承诺条件、秘密计划、关系转折和未决事项，不得以“双方讨论了某事”等泛化句替代具体事实。
